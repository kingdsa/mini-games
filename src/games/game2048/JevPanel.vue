...
import type { Game2048AiState } from './useGame2048'

const props = defineProps<{ ai: Game2048AiState }>()
...
const statusTone = computed(() => {
  ...
  if (props.ai.status === 'gameEnd') return 'restart'
  return 'ready'
})

const decision = computed(() => props.ai.decision)
const topOptions = computed(() => props.ai.ranked.slice(0, 4))
const history = computed(() => props.ai.history.slice(0, 6))
  ...
  <section class="panel glass jev">
    ...
    <template v-if="decision">
      <div class="jev__choice">
        <span class="jev__arrow" :class="`is-${decision.move.direction}`">
          {{ DIRECTION_META[decision.move.direction].arrow }}
        </span>
        <div class="jev__choice-main">
          <span class="jev__choice-label">{{ decision.move.label }}</span>
          <span class="jev__choice-sub">
            预估 +{{ decision.move.gain }} · 空格 ~{{ decision.move.avgEmpty }} · 最大 {{ decision.move.maxTile }}
          </span>
        </div>
      ...
      <div class="jev__rank">
        <h3 class="jev__subtitle">候选方向概率</h3>
        <ul>
          <li v-for="option in topOptions" :key="option.id" :class="{ 'is-chosen': option.chosen }">
            <span class="jev__rank-label">{{ option.label }}</span>
            <span class="jev__rank-bar"><i :style="{ width: `${Math.max(2, Math.min(100, option.probability * 100))}%` }" /></span>
            <span class="jev__rank-num">{{ (Math.min(option.probability * 100, 100)).toFixed(1) }}%</span>
          </li>
        </ul>
      </div>
    </template>
    <p v-else class="jev__empty">开启后 Jev 将接管每一次滑动，自动合并数字冲击 2048。</p>

    <div v-if="history.length" class="jev__history">
      <h3 class="jev__subtitle">决策历史</h3>
      <ul>
        <li v-for="(item, i) in history" :key="`${item.at}-${i}`">
          <span class="jev__history-arrow">{{ DIRECTION_META[item.move.direction].arrow }}</span>
          <span class="jev__history-label">{{ item.move.label }}</span>
          <span class="jev__history-score">+{{ item.move.gain }}</span>
    ...
    <div class="jev__stats">
      ...
      <div><span>得分</span><b>{{ ai.totalGain }}</b></div>
    </div>
  </section>
</template>
...
.jev__arrow {
  ...
  font-weight: 800;
  color: #1b0a13;
  background: linear-gradient(140deg, #ffe259, #ffb648);
  animation: arrowNudge 1.1s ease-in-out infinite;
}

.jev__arrow.is-up {
  animation-name: arrowNudgeUp;
}
.jev__arrow.is-down {
  animation-name: arrowNudgeDown;
}
.jev__arrow.is-left {
  animation-name: arrowNudgeLeft;
}
.jev__arrow.is-right {
  animation-name: arrowNudgeRight;
}
...
.jev__meter-fill {
  ...
  height: 100%;
  border-radius: 99px;
  background: linear-gradient(90deg, #ffb648, #ff4d8d);
...
.jev__rank-bar i {
  ...
  height: 100%;
  border-radius: 99px;
  background: linear-gradient(90deg, rgba(251, 191, 36, 0.9), rgba(255, 77, 141, 0.9));
