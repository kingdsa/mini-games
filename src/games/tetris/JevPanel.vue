...
const emit = defineEmits<{ toggle: []; saveKey: [key: string] }>()
...
const statusTone = computed(() => {
  ...
  if (props.ai.status === 'restarting') return 'restart'
  return 'ready'
})

const decision = computed(() => props.ai.decision)
const topOptions = computed(() => props.ai.ranked.slice(0, 14))
  ...
  <section class="panel glass jev">
    ...
    <template v-if="decision">
      <div class="jev__choice">
        <PiecePreview :type="decision.piece" :cell="12" />
        <div class="jev__choice-main">
          <span class="jev__choice-label">{{ decision.placement.label }}</span>
          <span class="jev__choice-sub">
            消 {{ decision.placement.linesCleared }} 行 · 洞 {{ decision.placement.holes }} · 高
            {{ decision.placement.maxHeight }}
          </span>
        </div>
      ...
      <div class="jev__meter">
        ...
        <p class="jev__meta">
          {{ decision.latencyMs }}ms · {{ tokens }} tokens · {{ formatTime(decision.at) }}
        </p>
      </div>

      <div class="jev__rank">
        <h3 class="jev__subtitle">落点概率分布</h3>
        <ul>
    ...
    <p v-else class="jev__empty">开启后 Jev 将接管每一块方块的落点，游戏自动无限进行。</p>

    <div v-if="history.length" class="jev__history">
      <h3 class="jev__subtitle">决策历史</h3>
      <ul>
        <li v-for="(item, i) in history" :key="`${item.at}-${i}`">
          <span class="jev__history-piece" :style="{ color: PIECE_SKINS[item.piece].glow }">{{ item.piece }}</span>
          <span class="jev__history-label">{{ item.placement.label }}</span>
          <span class="jev__history-conf">{{ Math.round(item.confidence * 100) }}%</span>
          <span class="jev__history-dot" :class="item.source === 'jev' ? 'is-jev' : 'is-fallback'" />
        </li>
      </ul>
    </div>

    <div class="jev__stats">
      <div><span>局数</span><b>{{ ai.games }}</b></div>
...
.jev__choice {
  ...
  padding: 10px;
  border-radius: var(--radius-sm);
  background: linear-gradient(140deg, rgba(251, 191, 36, 0.16), rgba(124, 92, 255, 0.12));
...
.jev__rank-bar i {
  ...
  height: 100%;
  border-radius: 99px;
  background: linear-gradient(90deg, rgba(124, 92, 255, 0.9), rgba(34, 211, 238, 0.9));
...
.jev__history-dot.is-jev {
  background: linear-gradient(120deg, #7df9ff, #a58bff);
}

.jev__history-dot.is-fallback {
  background: var(--warn);
}
